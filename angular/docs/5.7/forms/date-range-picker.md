...
CoreUI Date Range Picker supports both possibilities.
...
| `dayFormat` | `DayFormatType` | `'numeric'` | Set the format of day name. |
...
| `placeholder` | `string \| string[]` | `['Start date'` | Specifies short hints that are visible in start date and end date inputs. type string |
...
| `startDate` | `Date \| null` | `null` | Initial selected start date. |
...
| `startDateChange` | Emitted when `startDate` changes. |
