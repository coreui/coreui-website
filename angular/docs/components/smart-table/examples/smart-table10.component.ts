...
@Component({
  selector: 'docs-smart-table10',
  templateUrl: './smart-table10.component.html',
...
export class SmartTable10Component implements OnInit, OnDestroy {
  ...
  readonly props$: Observable<IParams> = combineLatest([
    ...
    this.sorterValue$.pipe(
      ...
      // map((sorterValue: ISorterValue) => {
      //   sorterValue.column === 'last_name' ? (sorterValue.column = 'customer') : sorterValue;
      //   return sorterValue;
      // })
    ),
    this.totalPages$
  ...
  handleSorterValueChange(sorterValue: ISorterValue) {
    ...
    if (sorterValue.column === 'customer') {
      sorterValue = { ...sorterValue, column: 'last_name' };
    }
