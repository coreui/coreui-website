...
Transform your expanding and collapsing navbar into an [offcanvas](https://coreui.io/angular/docs/components/offcanvas/) drawer. You can extend both the offcanvas default styles and use our `expand="*"` prop to create a custom navigation sidebar.
...
| `colorScheme` | `'dark' \| 'light'` | - | Sets the dark theme attribute on the host element. |
...
```jsx
import { NavbarTextComponent } from '@coreui/angular'
```
...
| `cNavbarToggler` | `CollapseDirective` | `undefined` | Reference to navbar collapse element (via # template variable) |
...
| Name | Type | Default | Description |
| --- | --- | --- | --- |
| `role` | `string` | `'button'` | Default role for navbar-brand. |
