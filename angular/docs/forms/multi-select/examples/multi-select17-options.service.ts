...
@Injectable()
export class MultiSelect17OptionsService {
  #users$: BehaviorSubject<IUser[]> = new BehaviorSubject<IUser[]>([]);
  readonly userList$ = this.#users$.asObservable();

  getUsers(country: string[]) {
    const users = this.#users.filter((user) => (country.length > 0 ? country.includes(user.country) : false));
  ...
  readonly countries = Array.from(new Set(this.#users.map((item) => item.country))).sort();
}
