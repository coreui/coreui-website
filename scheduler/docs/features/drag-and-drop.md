...
```html
<div id="schedulerDnd"></div><p id="schedulerDndLog" class="mt-2 mb-0 text-body-secondary"></p>
```
...
const element = document.getElementById('schedulerDnd')
const log = document.getElementById('schedulerDndLog')
...
element.addEventListener('eventChange.coreui.scheduler', event => {
  log.textContent = `${event.action}: ${event.event.title} → ${event.event.start}`
})
```
...
Every committed operation emits `eventChange.coreui.scheduler`:
...
standalone event. See [Recurrence](https://coreui.io/scheduler/docs/features/recurrence/) for the other
scopes.
