...
const onEventChange = (change: EventChangePayload) => {
  log.value = `${change.action}: ${change.event.title} → ${change.event.start}`
}
</script>
...
standalone event. See [Recurrence](https://coreui.io/scheduler/vue/docs/features/recurrence/) for the other
scopes.
