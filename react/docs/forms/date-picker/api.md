...
| `calendarDate` | `string \| Date \| null` | - | Defines the initial date or reference date for the React Calendar component. This date can be either a Date object or a string that can be converted into a valid date. |
...
| `cleaner` | `boolean` | - | Toggle visibility or set the content of the cleaner button. |
| `closeOnSelect` | `boolean` | - | If true the dropdown will be immediately closed after submitting the full date. |
...
| `date` | `string \| Date \| null` | - | Initial selected date. |
...
| `inputOnChangeDelay` | `number` | - | Defines the delay (in milliseconds) for the input field's onChange event. |
...
| `onDateChange` | `((date: string \| Date \| null, formatedDate?: string \| undefined) => void)` | - | Callback fired when the date changed. |
...
| `placeholder` | `string \| string[]` | `Select date` | Specifies short hints that are visible in start date and end date inputs. |
| `portal` | `boolean` | - | Generates dropdown menu using createPortal. |
| `previewDateOnHover` | `boolean` | - | Enable live preview of dates in input fields when hovering over calendar cells. |
...
| `rangesButtonsVariant` | `"outline" \| "ghost"` | - | Set the ranges button variant to an outlined button or a ghost button. |
