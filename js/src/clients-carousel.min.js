...
if(settings.classes.hasOwnProperty('type')){options.classes.type=_extends({},defaults.classes.type,settings.classes.type);}
if(settings.classes.hasOwnProperty('slide')){options.classes.slide=_extends({},defaults.classes.slide,settings.classes.slide);}
if(settings.classes.hasOwnProperty('arrow')){options.classes.arrow=_extends({},defaults.classes.arrow,settings.classes.arrow);}
if(settings.classes.hasOwnProperty('nav')){options.classes.nav=_extends({},defaults.classes.nav,settings.classes.nav);}}
...
Events.emit('run.after',_this.move);Glide.enable();});}},calculate:function calculate(){var move=this.move,length=this.length;var steps=move.steps,direction=move.direction;var viewSize=1;if(direction==='='){Glide.index=steps;return;}
...
warn('Invalid direction pattern ['+direction+steps+'] has been used');},isStart:function isStart(){return Glide.index<=0;},isEnd:function isEnd(){return Glide.index>=this.length;},isOffset:function isOffset(){var direction=arguments.length>0&&arguments[0]!==undefined?arguments[0]:undefined;if(!direction){return this._o;}
...
function Sizes(Glide,Components,Events){var Sizes={setupSlides:function setupSlides(){var width='216.73px';if(Math.max(document.documentElement.clientWidth,window.innerWidth||0)<=768){var width='99.06px';}
var slides=Components.Html.slides;for(var i=0;i<slides.length;i++){slides[i].style.width=width;}},setupWrapper:function setupWrapper(){Components.Html.wrapper.style.width=this.wrapperSize+'px';},remove:function remove(){var slides=Components.Html.slides;for(var i=0;i<slides.length;i++){slides[i].style.width='';}
...
return items;},append:function append(){var items=this.items;var _Components$Html=Components.Html,wrapper=_Components$Html.wrapper,slides=_Components$Html.slides;var half=Math.floor(items.length/2);var prepend=items.slice(0,half).reverse();var append=items.slice(half*-1).reverse();var width=Components.Sizes.slideWidth+'px';for(var i=0;i<append.length;i++){wrapper.appendChild(append[i]);}
...
createClass(Glide$$1,[{key:'mount',value:function mount(){var extensions=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{};return get(Glide$$1.prototype.__proto__||Object.getPrototypeOf(Glide$$1.prototype),'mount',this).call(this,_extends({},COMPONENTS,extensions));}}]);return Glide$$1;}(Glide);return Glide$1;})));var hexagonWidth=216.73;var mobileHegagonWidth=99.06;var currentVieportWidth=Math.max(document.documentElement.clientWidth,window.innerWidth||0);new Glide('.c-customers-carousel',{type:'carousel',autoplay:1000,animationDuration:1100,perView:Math.round(currentVieportWidth/hexagonWidth),breakpoints:{576:{perView:Math.round(576/mobileHegagonWidth)},768:{perView:Math.round(768/mobileHegagonWidth)},992:{perView:Math.round(992/hexagonWidth)},1200:{perView:Math.round(1200/hexagonWidth)},1380:{perView:Math.round(1380/hexagonWidth)}}}).mount()