...
const checkLicense=(e)=>{if(e.target.value.length===35){const xhr=new XMLHttpRequest()
xhr.open('POST','https://sales.coreui.io/check_license.php',true)
...
createAlert(e.target.parentElement,'success',`The license number is valid. Your ${last.support_level.toLowerCase()} support expires on ${valid}`)}else{createAlert(e.target.parentElement,'danger',`The license number is valid, but your ${last.support_level.toLowerCase()} support expired on ${valid}. To extend ${last.support_level.toLowerCase()} support please buy a PRO license.`)}}else{createAlert(e.target.parentElement,'danger','The license number is invalid.')}}
...
let timeout=null;document.getElementsByName('license')[0].addEventListener('keyup',e=>{clearTimeout(timeout)
timeout=setTimeout(()=>{checkLicense(e)},250)});form.onsubmit=event=>{event.preventDefault()
event.stopPropagation()
const target=event.currentTarget
const inputs=target.elements
if(target[0].checkValidity()===false){target.classList.add('c-was-validated')}else{const type=inputs['type'].value
...
data={type,name,email,timeline,budget,message}}else{const product=inputs['product'].value
const license=inputs['license'].value
data={type,name,email,product,license,message,supportActive,supportLevel}}
...
if(response.status===200&&response.ok===true){message.classList.add('c-alert','c-alert-success','c-mb-4')
message.innerHTML='Your message has been sent.'
target.prepend(message)
target.reset()}else{message.classList.add('c-alert','c-alert-danger','c-mb-4')
