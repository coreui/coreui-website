...
> ### Using embedded SVG
> 
> Inlining SVG as data URI requires to URL escape a few characters, most notably `` and `#`. That's why the `$breadcrumb-divider` variable is passed through our [`escape-svg()` Sass function](https://coreui.io/bootstrap/docs/customize/sass/#escape-svg). When using the CSS custom property, you need to URL escape your SVG on your own. Read [Kevin Weber's explanations on CodePen](https://codepen.io/kevinweber/pen/dXWoRw ) for detailed information on what to escape.
