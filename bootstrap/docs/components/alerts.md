...
In a similar vein, you can utilize [flexbox utilities](https://coreui.io/bootstrap/docs/utilities/flex/) and [CoreUI Icons](https://coreui.io/icons/) for crafting alerts that include icons. Based on your choice of icons and the content involved, you might consider incorporating additional utilities or custom styling.
...
- Add a [close button](https://coreui.io/bootstrap/docs/components/close-button//) along with the `.alert-dismissible` class, which provides extra padding to the right of the alert component and positions the `.close` button.
...
@mixin alert-variant($background, $border, $color) {
  --cui-alert-color: #{$color};
  --cui-alert-bg: #{$background};
  --cui-alert-border-color: #{$border};
  --cui-alert-link-color: #{shade-color($color, 20%)};
