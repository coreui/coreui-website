...
`cell-selection` gives the grid that, on top of the same active-cell substrate
[keyboard navigation](https://coreui.io/data-grid/vue/docs/features/keyboard-navigation/) already provides.
...
`cell-selection` implies `cell-navigation`: the active cell is the grid's single focus owner, and
...
Each copy fires ``@cell-copy`` with the exact text written, so you can mirror it
...
| --- | --- | --- | --- |
| `cell-selection` | `boolean` | `false` | Enables cell ranges, clipboard copy and select-all. Implies `cell-navigation`. |
...
[feature set](https://coreui.io/data-grid/vue/docs/customization/feature-sets/) — which is exactly what the lite
build does.
