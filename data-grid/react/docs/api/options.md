...
| --- | --- | --- | --- |
| `autoRowHeight` | `boolean \| undefined` | - | Measures each rendered row instead of trusting `rowHeight`, so rows grow with their content (wrapping cells, custom renderers). `rowHeight` stays the estimate for rows that have not been rendered yet. Requires virtualization. |
...
| `cellSelection` | `boolean \| undefined` | - | Spreadsheet-style cell ranges — shift-click, drag and shift+arrows, with Ctrl/Cmd+C copying the range as TSV and Ctrl/Cmd+A selecting everything. Implies `cellNavigation`. |
...
| `columnPinning` | `boolean \| Partial<ColumnPinningState> \| undefined` | - | Freezes columns (by `key`) to the start/end edge with sticky positioning; `true` enables the feature with no initial pins. |
...
| `columnVisibility` | `boolean \| ColumnVisibilityState \| undefined` | - | Enables hiding/showing columns via `column.toggleVisibility()`; an object sets the initial visibility. |
...
| `features` | `TableFeatures \| undefined` | - | The registered TanStack feature set — pass `dataGridLiteFeatures` (or a custom `tableFeatures({...})` set) so the app bundle only carries the features it uses; defaults to the full `dataGridFeatures`. |
...
| `infiniteScroll` | `boolean \| DataGridInfiniteScrollOptions \| undefined` | - | Appends each loaded page to the rows already in view instead of replacing them; requires `dataProvider` and `virtualization`, cannot be combined with `pagination`. |
...
| `onCellCopy` | `((event: { text: string; }) => void) \| undefined` | - | Fires after Ctrl/Cmd+C copies a cell selection, with the text written to the clipboard. |
...
| `onRowOrderChange` | `((event: CDataGridRowOrderEvent) => void) \| undefined` | - |  |
...
| `onSizingChange` | `((columnSizing: ColumnSizingState) => void) \| undefined` | - |  |
| `onSortingChange` | `((sorting: SortingState) => void) \| undefined` | - | Fires when sorting changes, with the TanStack sorting state. |
| `onVisibilityChange` | `((columnVisibility: ColumnVisibilityState) => void) \| undefined` | - | Fires when column visibility changes, with the TanStack columnVisibility state. |
...
| `rowOrder` | `boolean \| undefined` | - | Adds a drag handle per row for reordering; the grid emits `onRowOrderChange` with the reordered array instead of mutating `items`. |
| `rowSelection` | `boolean \| DataGridRowSelectionOptions \| undefined` | - | Checkbox selection with select-all and shift+click ranges. |
| `slots` | `Partial<CDataGridSlots<TFeatures>> \| undefined` | - | Replaces built-in chrome — each slot is a component rendered with `{ table, labels }`. |
...
| `tableRef` | `Ref<CDataGridTable<TFeatures>> \| undefined` | - | Ref that receives the TanStack table instance. |
