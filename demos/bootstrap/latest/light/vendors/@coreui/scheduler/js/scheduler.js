...
* Bundles rrule, BSD-3-Clause (https://github.com/jkbrzt/rrule/blob/master/LICENCE)
* Bundles temporal-polyfill, MIT (https://github.com/fullcalendar/temporal-polyfill/blob/main/LICENSE.txt)
* Bundles @tanstack/store and @tanstack/virtual-core, MIT (https://github.com/TanStack/virtual/blob/main/LICENSE)
*/
(function(global, factory) {
	...
	function parseText(text, language) {
		...
		function S() {
			...
			switch (ttr.symbol) {
				...
				case "sunday":
					options.freq = RRule$1.WEEKLY;
					options.byweekday = [RRule$1[ttr.symbol.substr(0, 2).toUpperCase()]];
	...
	*/
	var fromText = function(text, language) {
		if (language === void 0) language = ENGLISH;
		return new RRule$1(parseText(text, language) || void 0);
	};
	var common = [
		"count",
		"until",
	...
	function parseOptions(options) {
		...
		if (!(isPresent(opts.freq) && RRule$1.FREQUENCIES[opts.freq])) throw new Error("Invalid frequency: ".concat(opts.freq, " ").concat(options.freq));
		if (!opts.dtstart) opts.dtstart = new Date((/* @__PURE__ */ new Date()).setMilliseconds(0));
		if (!isPresent(opts.wkst)) opts.wkst = RRule$1.MO.weekday;
		...
		if (!(Boolean(opts.byweekno) || notEmpty(opts.byweekno) || notEmpty(opts.byyearday) || Boolean(opts.bymonthday) || notEmpty(opts.bymonthday) || isPresent(opts.byweekday) || isPresent(opts.byeaster))) switch (opts.freq) {
			...
			case RRule$1.MONTHLY:
				opts.bymonthday = opts.dtstart.getUTCDate();
				break;
			case RRule$1.WEEKLY: opts.byweekday = [getWeekday(opts.dtstart)];
		}
		...
		if (!isPresent(opts.byweekday)) opts.bynweekday = null;
			...
			for (var i = 0; i < opts.byweekday.length; i++) {
				...
				if (!wday.n || opts.freq > RRule$1.MONTHLY) byweekday.push(wday.weekday);
				else bynweekday.push([wday.weekday, wday.n]);
			}
			opts.byweekday = notEmpty(byweekday) ? byweekday : null;
			opts.bynweekday = notEmpty(bynweekday) ? bynweekday : null;
		}
		if (!isPresent(opts.byhour)) opts.byhour = opts.freq < RRule$1.HOURLY ? [opts.dtstart.getUTCHours()] : null;
		else if (isNumber(opts.byhour)) opts.byhour = [opts.byhour];
		if (!isPresent(opts.byminute)) opts.byminute = opts.freq < RRule$1.MINUTELY ? [opts.dtstart.getUTCMinutes()] : null;
		else if (isNumber(opts.byminute)) opts.byminute = [opts.byminute];
		if (!isPresent(opts.bysecond)) opts.bysecond = opts.freq < RRule$1.SECONDLY ? [opts.dtstart.getUTCSeconds()] : null;
	...
	function rebuildMonth(year, month, yearlen, mrange, wdaymask, options) {
		...
		} else if (options.freq === RRule$1.MONTHLY) ranges = [mrange.slice(month - 1, month + 1)];
	...
	function makeTimeset(ii, counterDate, options) {
		...
		if (freq >= RRule$1.HOURLY && notEmpty(byhour) && !includes(byhour, counterDate.hour) || freq >= RRule$1.MINUTELY && notEmpty(byminute) && !includes(byminute, counterDate.minute) || freq >= RRule$1.SECONDLY && notEmpty(bysecond) && !includes(bysecond, counterDate.second)) return [];
	...
	*/
	...
	function _addRule(rrule, collection) {
		if (!(rrule instanceof RRule$1)) throw new TypeError(String(rrule) + " is not RRule instance");
	...
	//#endregion
	//#region ../../node_modules/rrule/dist/esm/index.js
	...
	*/
	...
	function createInteractionMachine(options = {}) {
		...
		let press = null;
		let drag = null;
		const snapRound = (minutes) => Math.round(minutes / snap) * snap;
		...
		const movePreview = (origin, deltaColumns, rawDeltaMinutes) => {
			const duration = origin.endMinutes - origin.startMinutes;
			const startMinutes = clamp(origin.startMinutes + snapRound(rawDeltaMinutes), minMinutes, dayMinutes - duration);
			return {
		...
		const resizePreview = (origin, handle, rawDeltaMinutes) => {
			if (handle === "end") {
				const endMinutes = clamp(snapRound(origin.endMinutes + rawDeltaMinutes), origin.startMinutes + minDurationMinutes, dayMinutes);
				return {
			...
			const startMinutes = clamp(snapRound(origin.startMinutes + rawDeltaMinutes), minMinutes, origin.endMinutes - minDurationMinutes);
			return {
				column: origin.column,
				startMinutes,
		...
		const updateDrag = (pointer) => {
			...
			if (drag) {
				if (drag.mode === "move") setState({
					...
					preview: movePreview(drag.origin, deltaColumns, rawDeltaMinutes)
				});
				else {
					...
					setState({
						...
						occurrenceKey: drag.occurrenceKey,
						handle,
						preview: resizePreview(drag.origin, handle, rawDeltaMinutes)
					});
				}
				return;
			}
		...
		return {
			...
			grab(occurrenceKey, origin, mode = "move") {
				drag = {
					occurrenceKey,
					origin: {
						...origin,
				...
				if (mode === "move") setState({
					type: "drag-move",
					occurrenceKey,
					preview: movePreview(drag.origin, 0, 0)
				});
				else {
					...
					setState({
						...
						occurrenceKey,
						handle,
						preview: resizePreview(drag.origin, handle, 0)
					});
				}
			},
			nudge(delta) {
	...
	*/
	...
	function createRenderer(element, callbacks) {
		...
		const dialogHost = el("div", "scheduler-dialog-backdrop", "event-dialog");
		dialogHost.addEventListener("keydown", (event) => {
			if (event.key !== "Tab") return;
			const focusables = [...dialogHost.querySelectorAll("input, select, button")];
	...
	var Scheduler = class extends BaseComponent {
		...
		_handleEventKeydown(event) {
			...
			if (event.key === "Enter" || event.key === " ") {
				...
				this._machine.grab(key, origin, event.shiftKey && this._isTimeGrid() ? "resize-end" : "move");
				this._renderer.announce(interpolate(this._labels.eventGrabbed, { title: this._occurrenceByKey(key)?.event.title ?? "" }));
				return;
			}
			if (!grabbed) return;
			const nudge = {
		...
		_handleTimelineCommit(commit, occurrence, scope) {
			...
			const targetResource = model.rows[commit.action === "move" ? preview.column : preview.column]?.resource;
			let start;
			let end;
			if (occurrence.allDay) {
				const startDay = Math.round(preview.startMinutes / MINUTES_IN_DAY);
				const spanDays = Math.max(Math.round((preview.endMinutes - preview.startMinutes) / MINUTES_IN_DAY), 1);
				const startDate = model.days[0].add({ days: startDay });
				const event = {
					...occurrence.event,
					start: startDate.toString(),
					end: startDate.add({ days: spanDays }).toString(),
					resourceId: commit.action === "move" ? targetResource?.id : occurrence.event.resourceId
				};
				this._applyPatch({
			...
			if (commit.action === "move") {
				start = this._timelineZoned(preview.startMinutes);
				end = start ? start.add(exactDuration(occurrence.start, occurrence.end)) : null;
			} else {
